...
ll solve2(ll x) {
	...
	FOR(ts, 1, 109) {
		...
		FOR(i, 13) {
            // Resets dp[0] or dp[1] depending on the parity of i
			...
			FOR(j, 2) {
				...
				FOR(s, 109) {
					...
					FOR(p, 109) {
                        // If the value of dp at that location is zero its skipped
                        // the i&1^1 inverts the parity of i
						if(!dp[i&1^1][j][s][p])
							continue;
						...
						FOR(nd, 10) {
                            ...
                            // Adds the dp value in inverted parity of i, j, s, p
                            // to the value in parity of i, new digit < digit + j,
                            // sum + new digit, pt of p new digit
							dp[i&1][nd<d+j][s+nd][pt[p][nd]]+=dp[i&1^1][j][s][p];
                            // if new digit isn't zero and
                            // new digit < digit + j or leftover total > 9 and
                            // total + newdigit == total sum and
                            // the product divided by the new digit is 1
                            if(nd&&(nd<d+j||x2>9)&&s+nd==ts&&pt[p][nd]==1)
                                // The value in dp reverse parity of i, j, s, p
